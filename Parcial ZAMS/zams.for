...
c
      WRITE (6,1004)
 1004 FORMAT (' THE TOTAL MASS IS (IN MSUN) ')
      READ (5,*) AMASS
      WRITE (6,1005)
 1005 FORMAT (' X(Hydrogen) AND Y(Helium) ARE  ')
      READ (5,*) X, Y
      WRITE (6,1000)
 1000 FORMAT (' GUESS FOR CENTRAL PRESSURE (IN CGS)')
      READ (5,*) PC
      WRITE (6,1001)
 1001 FORMAT (' GUESS FOR CENTRAL TEMPERATURE  (IN K)')
      READ (5,*) TC
      WRITE (6,1002)
 1002 FORMAT (' GUESS FOR TOTAL RADIUS  (IN CM)')
      READ (5,*) R
      WRITE (6,1003)
 1003 FORMAT (' GUESS FOR TOTAL LUMINOSITY (IN LSUN)')
      READ (5,*) AL
...
c statement.
      WRITE (6,1006)
 1006 FORMAT (' WHAT IS YOUR OUTPUT FILE NAME?  ')
      READ (5,1007) OUTFILE
 1007 FORMAT (A20)
...
c to be used for pulsation calculations.
      WRITE (6,1008)
 1008 FORMAT (' DO YOU WANT PULSATION OUTPUT? (Y/N)')
      READ (5,1007) YORN
...
c Compute the model quantities at the mesh points.
     ...
     *      CHIT(IGO), CHIRHO(IGO), G3(IGO))
         ...
         CALL EPSILON2 (YGO, RHO(IGO), EPSGEN(IGO))
...
c Compute the model quantities at the mesh points.)
     ...
     *      CHIT(NIN), CHIRHO(NIN), G3(NIN))
         ...
         CALL EPSILON2 (YGO, RHO(NIN), EPSGEN(NIN))
c Convective or radiative?
...
c Compute the pressure at the next point.
      ...
      CALL EPSILON2 (YGO, RHO(1), EPSGEN(1))
...
c ***********************************************
c ***********************************************
        SUBROUTINE EPSILON2 (YGO, RHO, EPS)
c ***********************************************
